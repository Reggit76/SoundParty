...
def create_event_route(event_data: EventCreate, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
...
def update_event_route(event_id: int, event_data: EventCreate, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
...
def delete_event_route(event_id: int, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
