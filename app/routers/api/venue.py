...
def create_venue_route(venue_data: VenueCreate, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
...
def update_venue_route(venue_id: int, venue_data: VenueCreate, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
...
def delete_venue_route(venue_id: int, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
