...
def create_team_route(team_data: TeamCreate, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
...
def update_team_route(team_id: int, team_data: TeamCreate, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
...
def delete_team_route(team_id: int, conn: psycopg2.extensions.connection = Depends(get_db)):
    try:
